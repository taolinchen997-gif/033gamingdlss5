...
本包只给燕云十六声用（国服 yysls.exe；国际版 Where Winds Meet，wwm.exe）。下面是包里真实带着的文件，以及编进 033 自己程序里的第三方代码；原始许可放在同目录对应文件夹，不能用 033 的说明替代它们。`bundled/` 里是从编出这些程序的源码树里原样收集的全部许可原文。
