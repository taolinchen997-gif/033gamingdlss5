...
inline K033_NrSettings nr_defaults(bool enabled=true){return {sizeof(K033_NrSettings),2,uint32_t(enabled),1,1,{nr_default_layer(),nr_default_layer(),nr_default_layer()},{0,0,0},0};}
...
inline bool nr_settings(const K033_NrSettings& c){
    if(c.size!=sizeof(c)||c.version!=2||c.enabled>1||!c.layers||c.layers>3||!c.appearance_epoch||c.reserved)return false;
    for(unsigned i=0;i<3;++i)if(!nr_layer(c.layer[i])||!nr_sr_work(i,c.sr_work[i]))return false;return true;
}
