...
inline const std::array<std::string,27>& nr_keys(){
    static const auto names=[] {std::array<std::string,27> a{};a[0]="033_nr_version";a[1]="nr_enabled";a[2]="nr_layers";
        const char* fields[]={"style","preset","auto_mask","ui_correction","intensity","local_structure","local_tone","global_tone"};
        for(unsigned i=0;i<3;++i)for(unsigned j=0;j<8;++j)a[3+i*8+j]="nr_layer"+std::to_string(i+1)+"_"+fields[j];return a;}();return names;
}
...
template<class Char>bool read_settings(const Char* path,K033_Settings& out,K033_NrSettings& nr_out){
    ...
    char line[1024];bool seen[10]={},seen_nr[27]={},ok=true,has_nr=false;auto s=k033::defaults();auto nr=k033::nr_defaults(false);
    while(ok&&std::fgets(line,sizeof(line),f)){
        ...
        unsigned index=10,nindex=27;for(unsigned i=0;i<10;++i)if(!std::strcmp(line,keys[i])){index=i;break;}
        if(index==10)for(unsigned i=0;i<27;++i)if(names[i]==line){nindex=i;break;}
        if(index==10&&nindex==27){if(!std::strncmp(line,"nr_",3))ok=false;continue;}
        ...
        if(index<10){if(seen[index]){ok=false;break;}seen[index]=true;
            ...
            if(nindex<3){if(!integer(v,3)){ok=false;break;}
...
template<class Char>bool write_groups(const Char* path,const K033_Settings* s,const K033_NrSettings* nr){
    ...
    std::array<const char*,37> owned{};unsigned count=0;
