...
inline constexpr unsigned NrKeyCount=37;
inline const std::array<std::string,NrKeyCount>& nr_keys(){
    static const auto names=[] {std::array<std::string,NrKeyCount> a{};a[0]="033_nr_version";a[1]="nr_enabled";a[2]="nr_layers";
        ...
        for(unsigned i=0;i<3;++i)a[27+i]="033_nr_sr_work"+std::to_string(i+1);a[30]="033_post_beauty";
        // Old cores ignore and preserve extensions outside the strict nr_ namespace.
        for(unsigned i=0;i<3;++i)a[31+i]="033_nr_skin_structure"+std::to_string(i+1);
        a[34]="033_nr_skin_lift";a[35]="033_nr_final_clarity";a[36]="033_nr_natural_look";return a;}();return names;
}
...
template<class Char>bool read_settings(const Char* path,K033_Settings& out,K033_NrSettings& nr_out){
    ...
    char line[1024];bool seen[10]={},seen_nr[NrKeyCount]={},ok=true,has_nr=false;auto s=k033::defaults();auto nr=k033::nr_defaults(false);
    while(ok&&std::fgets(line,sizeof(line),f)){
        ...
        unsigned index=10,nindex=NrKeyCount;for(unsigned i=0;i<10;++i)if(!std::strcmp(line,keys[i])){index=i;break;}
        if(index==10)for(unsigned i=0;i<NrKeyCount;++i)if(names[i]==line){nindex=i;break;}
        if(index==10&&nindex==NrKeyCount){if(!std::strncmp(line,"nr_",3))ok=false;continue;}
        if(nindex==30)continue; // Retired extension: ignore even malformed historical values.
        ...
        if(index<10){if(seen[index]){ok=false;break;}seen[index]=true;
            ...
            if(nindex>=31){if(nindex==36)nr.natural_look=v;else if(nindex==35)nr.final_clarity=v;else if(nindex==34)nr.skin_lift=v;else nr.skin_structure[nindex-31]=v;}
            else if(nindex>=27){if(!integer(v,200)||!k033::nr_sr_work(nindex-27,uint32_t(v))){ok=false;break;}nr.sr_work[nindex-27]=uint32_t(v);}
...
inline void print_nr(FILE* f,const K033_NrSettings& nr){
    ...
    _fprintf_l(f,"033_nr_skin_structure1=%.9g\n033_nr_skin_structure2=%.9g\n033_nr_skin_structure3=%.9g\n033_nr_skin_lift=%.9g\n",numeric_locale(),
        nr.skin_structure[0],nr.skin_structure[1],nr.skin_structure[2],nr.skin_lift);
    _fprintf_l(f,"033_nr_natural_look=%.9g\n",numeric_locale(),nr.natural_look);
    _fprintf_l(f,"033_nr_final_clarity=%.9g\n",numeric_locale(),nr.final_clarity);
...
template<class Char>bool write_groups(const Char* path,const K033_Settings* s,const K033_NrSettings* nr){
    ...
    std::array<const char*,10+NrKeyCount> owned{};unsigned count=0;
