...
bool MenuOverlayVk::QueuePresent(VkQueue queue, VkPresentInfoKHR* pPresentInfo)
{
    ...
    // No vendor UI submission or wait-semaphore replacement; keep normal presentation.
    return true;
#endif
    LOG_FUNC();
...
{
#ifdef K033_BETA2_RESHADE_HOST
    // Do not allocate a second UI backend; the actual Vulkan hooks are unchanged.
    return;
#endif
    LOG_FUNC();
