...
#include <integration/EmbeddedUi033.h>

class ScopedIndent
{
  public:
    explicit ScopedIndent(float indent = 16.0f) : m_indent(indent) { Ui033::Indent(m_indent); }

    ~ScopedIndent() { Ui033::Unindent(m_indent); }
...
class ScopedCollapsingHeader
{
  public:
    explicit ScopedCollapsingHeader(const char* label, ImGuiTreeNodeFlags flags = 0)
    {
        ...
        Ui033::BeginChild("##CollapsingHeaderChild", ImVec2(0, 0), ImGuiChildFlags_AutoResizeY,
                          ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);

        _headerOpen = Ui033::CollapsingHeader(label, flags);
        _active = true;
    }
...
class MenuCommon
{
  ...
  public:
    ...
    static int RenderEmbedded(const ui033abi::Api*);
    static int RenderSupplemental(const ui033abi::Api*);
