...
bool DLSSFeatureDx12::EvaluateInternal(ID3D12GraphicsCommandList* InCommandList, NVSDK_NGX_Parameter* InParameters)
{
    ...
    if (NVNGXProxy::D3D12_EvaluateFeature() != nullptr)
    {
        ...
        EvaluationResult=nvResult;

        if (nvResult != NVSDK_NGX_Result_Success)
        {
            unsigned int inputW=0,inputH=0;
            InParameters->Get(NVSDK_NGX_Parameter_DLSS_Render_Subrect_Dimensions_Width,&inputW);
            InParameters->Get(NVSDK_NGX_Parameter_DLSS_Render_Subrect_Dimensions_Height,&inputH);
            LOG_ERROR("_EvaluateFeature result: {:X}; input={}x{} target={}x{} frame={}",
                (unsigned int)nvResult,inputW,inputH,TargetWidth(),TargetHeight(),_frameCount);
            return false;
        }
    }
    else
