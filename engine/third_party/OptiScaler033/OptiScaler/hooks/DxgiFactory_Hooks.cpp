...
void DxgiFactoryHooks::HookToFactory(IDXGIFactory* pFactory)
{
    ...
    loader033::TryHook attempt(installing);
    if (!attempt || o_EnumAdapters != nullptr)
        return;
    ...
    // A foreign/nested transaction is not ours to attach to, commit or clean up.
    // Leave the cached pointers untouched so a later factory can retry.
    const auto beginResult = DetourTransactionBegin();
    if (beginResult != NO_ERROR)
    {
        LOG_WARN("Deferring IDXGIFactory hooks: transaction busy ({:X})", beginResult);
        return;
    }
