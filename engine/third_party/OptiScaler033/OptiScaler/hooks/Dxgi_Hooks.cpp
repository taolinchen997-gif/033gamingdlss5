...
void DxgiHooks::Hook()
{
    ...
    bool imageFgPrepared=false;
    if(auto imageFg=ufg033client::Get()){ufg033abi::Status status;imageFgPrepared=imageFg->read(&status)&&status.prepared;}
    if (!imageFgPrepared && !Config::Instance()->OverlayMenu.value_or_default() &&
