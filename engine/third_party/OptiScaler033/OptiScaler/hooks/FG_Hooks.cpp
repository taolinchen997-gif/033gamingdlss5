...
{
    if(fgroute033::UniversalBoot())return false;
...
// The image-only path has no dependency on NGX/FSR input calls. It owns a
// separate swapchain and remains mutually exclusive with translated/native FG.
static HRESULT TryImage033(IDXGIFactory* factory,IUnknown* device,HWND hwnd,const DXGI_SWAP_CHAIN_DESC1* desc,
                           const DXGI_SWAP_CHAIN_FULLSCREEN_DESC* full,IDXGISwapChain** output)
{
    auto api=ufg033client::Get();if(!api||!device||!desc||!output||!fgroute033::CanGenerate()||State::Instance().activeFgInput!=FGInput::NoFG)return E_NOINTERFACE;
    ufg033abi::Status status;if(!api->read(&status)||!status.prepared||status.available)return E_NOINTERFACE;
    ID3D12CommandQueue* queue=nullptr;if(FAILED(device->QueryInterface(IID_PPV_ARGS(&queue))))return E_NOINTERFACE;
    IUnknown* realDevice=nullptr;if(!Util::CheckForRealObject(__FUNCTION__,queue,&realDevice))realDevice=queue;
    IDXGIFactory* realFactory=nullptr;if(!Util::CheckForRealObject(__FUNCTION__,factory,reinterpret_cast<IUnknown**>(&realFactory)))realFactory=factory;
    IDXGISwapChain4* imageSwapchain=nullptr;ufg033abi::Create c;c.factory=realFactory;c.queue=realDevice;c.hwnd=hwnd;c.desc=desc;c.fullscreen=full;c.output=reinterpret_cast<void**>(&imageSwapchain);
    HRESULT result;
    {ScopedSkipParentWrapping skipParent;ScopedSkipDxgiLoadChecks skipChecks;result=api->create(&c);}
    if(SUCCEEDED(result)&&imageSwapchain){
        *output=new WrappedIDXGISwapChain4(imageSwapchain,realDevice,hwnd,desc->Flags,false,true);
        State::Instance().currentSwapchain=*output;State::Instance().currentWrappedSwapchain=*output;
        State::Instance().currentRealSwapchain=imageSwapchain;State::Instance().swapchainInteropApi=SwapchainInteropApi::None;
        (*output)->GetDesc(&State::Instance().currentSwapchainDesc);
        LOG_INFO("033 image-only FG swapchain ready; starts disabled; native input not required");
    }else LOG_WARN("033 image-only swapchain unavailable: {:X}; keeping original game swapchain",(UINT)result);
    queue->Release();return result;
}

HRESULT FGHooks::CreateSwapChain(IDXGIFactory* pFactory, IUnknown* pDevice, DXGI_SWAP_CHAIN_DESC* pDesc,
                                 IDXGISwapChain** ppSwapChain)
{
    if(pDesc){DXGI_SWAP_CHAIN_DESC1 d{};d.Width=pDesc->BufferDesc.Width;d.Height=pDesc->BufferDesc.Height;d.Format=pDesc->BufferDesc.Format;
        d.Stereo=FALSE;d.SampleDesc=pDesc->SampleDesc;d.BufferUsage=pDesc->BufferUsage;d.BufferCount=pDesc->BufferCount;d.SwapEffect=pDesc->SwapEffect;d.Flags=pDesc->Flags;
        DXGI_SWAP_CHAIN_FULLSCREEN_DESC fs{};fs.RefreshRate=pDesc->BufferDesc.RefreshRate;fs.ScanlineOrdering=pDesc->BufferDesc.ScanlineOrdering;fs.Scaling=pDesc->BufferDesc.Scaling;fs.Windowed=pDesc->Windowed;
        if(SUCCEEDED(TryImage033(pFactory,pDevice,pDesc->OutputWindow,&d,&fs,ppSwapChain)))return S_OK;}
    if (!CheckForFGStatus())
    {
...
{
    if(!pRestrictToOutput&&SUCCEEDED(TryImage033(pFactory,pDevice,hWnd,pDesc,pFullscreenDesc,reinterpret_cast<IDXGISwapChain**>(ppSwapChain))))return S_OK;
    if (!CheckForFGStatus())
    {
        LOG_WARN("Can't init FG Feature or invalid FGOutput setting!");
        return E_NOINTERFACE;
