...
sl::Result StreamlineHooks::hkslInit(const sl::Preferences& pref, uint64_t sdkVersion)
{
    ...
    const bool validFGList=pref.numFeaturesToLoad<=64 && (!pref.numFeaturesToLoad||pref.featuresToLoad);
    const bool requestsFG=fgcap033::Requests(pref.featuresToLoad,pref.numFeaturesToLoad,sl::kFeatureDLSS_G);
    const bool compatibleFG=o_slGetFeatureFunction && o_slEvaluateFeature;
    const auto observeFG=[&](sl::Result result){
        fgroute033::nativeCapability.Observe(fgcap033::Streamline(validFGList,requestsFG,compatibleFG,result==sl::Result::eOk));
        return result;
    };
    ...
    if(fgroute033::UniversalBoot()) {
        fgroute033::streamlineExcluded=false;
        // Exclude only DLSS-G before any presentation ownership is created.
        // SR, NR and Reflex keep the caller's feature list and options.
        if(localPref.numFeaturesToLoad && !localPref.featuresToLoad)
            return sl::Result::eErrorInvalidParameter;
        auto features=fgroute033::WithoutFramegen(localPref.featuresToLoad,localPref.numFeaturesToLoad,sl::kFeatureDLSS_G);
        localPref.featuresToLoad=features.data();localPref.numFeaturesToLoad=uint32_t(features.size());
        auto result=o_slInit(localPref,sdkVersion);
        fgroute033::streamlineExcluded=result==sl::Result::eOk && fgroute033::streamlineHooksReady.load();
        LOG_INFO("033 universal route: DLSS-G excluded from slInit; result={} guard={}",int(result),fgroute033::streamlineExcluded.load());
        return observeFG(result);
    }
    ...
    if (State::Instance().activeFgInput == FGInput::DLSSG || State::Instance().activeFgOutput == FGOutput::DLSSG)
    {
        ...
        return observeFG(o_slInit(localPref, sdkVersion));
    }
...
{
    if(fgroute033::UniversalBoot()&&feature==sl::kFeatureDLSS_G)return sl::Result::eErrorFeatureNotSupported;
...
{
    if(fgroute033::UniversalBoot()&&feature==sl::kFeatureDLSS_G){loaded=false;return sl::Result::eOk;}
    if (feature == sl::kFeatureDLSS_G)
    {
        loaded = true;
        return sl::Result::eOk;
...
{
    return fgroute033::FeatureCall(fgroute033::UniversalBoot(),feature==sl::kFeatureDLSS_G,
        sl::Result::eErrorFeatureNotSupported,[&]{return o_slSetFeatureLoaded(feature,loaded);});
}

sl::Result StreamlineHooks::hkslGetFeatureFunction(sl::Feature feature, const char* functionName, void*& function)
{
    if(fgroute033::UniversalBoot()&&feature==sl::kFeatureDLSS_G){function=nullptr;return sl::Result::eErrorFeatureMissing;}
    if (feature == sl::kFeatureDLSS_G)
    {
...
bool StreamlineHooks::hkslInit_sl1(const sl1::Preferences& pref, int applicationId)
{
    ...
    const bool universal=fgroute033::UniversalBoot();
    if(universal)fgroute033::streamlineExcluded=false;
    const bool known=fgroute033::LegacyPreferences(localPref.featuresToLoad,localPref.numFeaturesToLoad,localPref.ext!=nullptr);
    const bool result=o_slInit_sl1(localPref, applicationId);
    if(known&&result)fgroute033::nativeCapability.Observe(fgcap033::OldInterface);
    if(universal){
        fgroute033::streamlineExcluded=fgroute033::LegacyExcluded(known,result,fgroute033::streamlineHooksReady.load());
        LOG_INFO("033 universal SL1: knownFeatures={} init={} lateGuards={} allowed={}",known,result,
            fgroute033::streamlineHooksReady.load(),fgroute033::streamlineExcluded.load());
    }
    return result;
}

bool StreamlineHooks::hkslIsFeatureSupported_sl1(sl1::Feature feature,uint32_t* adapterBitMask){
    if(fgroute033::UniversalBoot()&&!fgroute033::LegacyFeature(unsigned(feature))){if(adapterBitMask)*adapterBitMask=0;return false;}
    return o_slIsFeatureSupported_sl1(feature,adapterBitMask);
}
bool StreamlineHooks::hkslIsFeatureEnabled_sl1(sl1::Feature feature){
    return fgroute033::LegacyCall(fgroute033::UniversalBoot(),unsigned(feature),[&]{return o_slIsFeatureEnabled_sl1(feature);});
}
bool StreamlineHooks::hkslSetFeatureEnabled_sl1(sl1::Feature feature,bool enabled){
    return fgroute033::LegacyCall(fgroute033::UniversalBoot(),unsigned(feature),[&]{return o_slSetFeatureEnabled_sl1(feature,enabled);});
}
...
bool StreamlineHooks::hkslEvaluateFeature_sl1(sl1::CommandBuffer* cmdBuffer, sl1::Feature feature, uint32_t frameIndex,
                                              uint32_t id)
{
    if(fgroute033::UniversalBoot()&&!fgroute033::LegacyFeature(unsigned(feature)))return false;
...
sl::Result StreamlineHooks::hkslDLSSGSetOptions(const sl::ViewportHandle& viewport, const sl::DLSSGOptions& options)
{
    ...
    if(fgroute033::UniversalBoot())return sl::Result::eErrorFeatureNotSupported;
    auto* nativeConfig=Config::Instance();
    if(!nativeConfig->FGEnabled.value_or_default() &&
       nativeConfig->FGInput.value_or_default()==FGInput::NoFG &&
       nativeConfig->FGOutput.value_or_default()==FGOutput::NoFG &&
       !nativeConfig->FGDLSSGOverrideForceDMFG.value_or_default() &&
       !nativeConfig->FGDLSSGOverrideInterpolationCount.has_value() &&
       !nativeConfig->FGDLSSGFramerateTargetDMFG.has_value()) {
        static std::atomic<unsigned> previous{~0u};
        const unsigned mode=unsigned(options.mode);
        if(previous.exchange(mode)!=mode)
            LOG_INFO("033 native FG passthrough: incoming_mode={} generated={} struct_version={}; no OptiScaler mode/count override",mode,options.numFramesToGenerate,options.structVersion);
...
{
    fgroute033::streamlineHooksReady=false;fgroute033::streamlineExcluded=false;
    LOG_FUNC();
    ...
    if(fgroute033::UniversalBoot()){
        ...
        if(o_slGetFeatureFunction)DetourDetach(&(PVOID&)o_slGetFeatureFunction,hkslGetFeatureFunction);
    }
    if (o_slSetTag)
    ...
    if(fgroute033::UniversalBoot()){
        if(o_slIsFeatureSupported_sl1)DetourDetach(&(PVOID&)o_slIsFeatureSupported_sl1,hkslIsFeatureSupported_sl1);
        if(o_slIsFeatureEnabled_sl1)DetourDetach(&(PVOID&)o_slIsFeatureEnabled_sl1,hkslIsFeatureEnabled_sl1);
        if(o_slSetFeatureEnabled_sl1)DetourDetach(&(PVOID&)o_slSetFeatureEnabled_sl1,hkslSetFeatureEnabled_sl1);
    }
    ...
    {
        if(fgroute033::UniversalBoot()){
            o_slIsFeatureSupported=nullptr;o_slIsFeatureLoaded=nullptr;o_slSetFeatureLoaded=nullptr;o_slGetFeatureFunction=nullptr;
        }
        o_slInit = nullptr;
        ...
        o_slIsFeatureSupported_sl1=nullptr;o_slIsFeatureEnabled_sl1=nullptr;o_slSetFeatureEnabled_sl1=nullptr;
...

    {
        ...
        if (sl_version.major >= 2)
        {
            ...
            if (o_slInit != nullptr)
            {
                ...
                if(fgroute033::UniversalBoot()) {
                    if(o_slIsFeatureSupported)DetourAttach(&(PVOID&)o_slIsFeatureSupported,hkslIsFeatureSupported);
                    if(o_slIsFeatureLoaded)DetourAttach(&(PVOID&)o_slIsFeatureLoaded,hkslIsFeatureLoaded);
                    if(o_slSetFeatureLoaded)DetourAttach(&(PVOID&)o_slSetFeatureLoaded,hkslSetFeatureLoaded);
                    if(o_slGetFeatureFunction)DetourAttach(&(PVOID&)o_slGetFeatureFunction,hkslGetFeatureFunction);
                }
                ...
                if (detourResult != NO_ERROR)
                {
                    ...
                    fgroute033::streamlineHooksReady=false;fgroute033::streamlineExcluded=false;o_slSetFeatureLoaded=nullptr;
                    o_slSetTag = nullptr;
                    o_slSetTagForFrame = nullptr;
                    o_slInit = nullptr;
                    o_slEvaluateFeature = nullptr;
                ...
                if(fgroute033::UniversalBoot())fgroute033::streamlineHooksReady=detourResult==NO_ERROR &&
                    o_slInit && o_slEvaluateFeature && o_slIsFeatureSupported && o_slIsFeatureLoaded && o_slSetFeatureLoaded && o_slGetFeatureFunction;
            }
        }
        else if (sl_version.major == 1)
        {
            ...
            o_slEvaluateFeature_sl1 = reinterpret_cast<decltype(&sl1::slEvaluateFeature)>(
                KernelBaseProxy::GetProcAddress_()(slInterposer, "slEvaluateFeature"));
            if(fgroute033::UniversalBoot()){
                o_slIsFeatureSupported_sl1=reinterpret_cast<decltype(&sl1::slIsFeatureSupported)>(KernelBaseProxy::GetProcAddress_()(slInterposer,"slIsFeatureSupported"));
                o_slIsFeatureEnabled_sl1=reinterpret_cast<decltype(&sl1::slIsFeatureEnabled)>(KernelBaseProxy::GetProcAddress_()(slInterposer,"slIsFeatureEnabled"));
                o_slSetFeatureEnabled_sl1=reinterpret_cast<decltype(&sl1::slSetFeatureEnabled)>(KernelBaseProxy::GetProcAddress_()(slInterposer,"slSetFeatureEnabled"));
            }
            ...
            if (o_slInit_sl1 || o_slSetTag_sl1 || o_slSetConstants_interposer_sl1 || o_slEvaluateFeature_sl1)
            {
                ...
                if(fgroute033::UniversalBoot()){
                    if(o_slIsFeatureSupported_sl1)DetourAttach(&(PVOID&)o_slIsFeatureSupported_sl1,hkslIsFeatureSupported_sl1);
                    if(o_slIsFeatureEnabled_sl1)DetourAttach(&(PVOID&)o_slIsFeatureEnabled_sl1,hkslIsFeatureEnabled_sl1);
                    if(o_slSetFeatureEnabled_sl1)DetourAttach(&(PVOID&)o_slSetFeatureEnabled_sl1,hkslSetFeatureEnabled_sl1);
                    if(o_slEvaluateFeature_sl1)DetourAttach(&(PVOID&)o_slEvaluateFeature_sl1,hkslEvaluateFeature_sl1);
                }
                ...
                if (IsSL1AndFGActive())
                {
                    ...
                    if (o_slEvaluateFeature_sl1 && !fgroute033::UniversalBoot())
                ...
                if(fgroute033::UniversalBoot())fgroute033::streamlineHooksReady=detourResult==NO_ERROR &&
                    o_slInit_sl1 && o_slEvaluateFeature_sl1 && o_slIsFeatureSupported_sl1 && o_slIsFeatureEnabled_sl1 && o_slSetFeatureEnabled_sl1;
            }
        }
    }
