...
{
#ifdef K033_MONOLITHIC_ENGINE
    const bool nativePath=!Config::Instance()->FGEnabled.value_or_default() &&
        Config::Instance()->FGInput.value_or_default()==FGInput::NoFG &&
        Config::Instance()->FGOutput.value_or_default()==FGOutput::NoFG &&
        !fakenvapi::isUsingAsMainNvapi();
    if(nativereflex::SkipCapWrite(nativePath,fps,_minimumIntervalUs)){
        LOG_INFO("033 native Reflex: no cap override owned; skipped redundant zero-cap SetSleepMode");
        return;
    }
#endif
...
#ifdef K033_MONOLITHIC_ENGINE
        if(fps==0.0f)temp.minimumIntervalUs=nativereflex::ReleaseCap(nativePath,_lastSleepParams.minimumIntervalUs);
#endif
    ...
    if (_lastVkSleepDev != nullptr)
    {
        ...
        temp.minimumIntervalUs = _minimumIntervalUs;
#ifdef K033_MONOLITHIC_ENGINE
        if(fps==0.0f)temp.minimumIntervalUs=nativereflex::ReleaseCap(nativePath,_lastVkSleepParams.minimumIntervalUs);
#endif
