...
static void CheckWorkingMode()
{
    ...
    do
    {
        ...
        // optiscaler.dll
        if (lCaseFilename == "optiscaler.dll" || lCaseFilename == "033-render-core.dll"
#ifdef K033_BETA2_RESHADE_HOST
            || lCaseFilename == "033-engine.dll"
#endif
            )
        {
            if (!_passThruMode)
...
#ifdef K033_BETA2_RESHADE_HOST
static BOOL APIENTRY Beta2VendorEntry(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
#else
...
#endif
{
    ...
    case DLL_PROCESS_ATTACH:
    {
...
#ifdef K033_BETA2_RESHADE_HOST
        // ReShade owns the only game proxy. This is a managed child module;
        // dependencies belong beside it, never a guessed game-root folder.
        Config::Instance()->MainDllPath.set_volatile_value(Util::DllPath().parent_path());
        Config::Instance()->PluginPath.set_volatile_value(Util::DllPath().parent_path() / L"plugins");
        Config::Instance()->LoadAsiPlugins.set_volatile_value(false);
        ...
        // Upstream update notices describe a different binary. This engine is
        // updated as a complete 033 build with its matching adapter and ABI.
        Config::Instance()->CheckForUpdate.set_volatile_value(false);
#endif

        // This override is process-local. SaveIni retains the native route's
        // original input/output/options, including its native multiplier.
        ...
        LOG_INFO("033 frame generation boot route: {}",fgroute033::UniversalBoot()?"universal":"native/preserved");
...
#if !defined(_DEBUG) && !defined(K033_BETA2_RESHADE_HOST)
        // The product does not start a background worker for standalone vendor toasts.
...
#ifdef K033_BETA2_RESHADE_HOST
// The host prepares the one shared settings owner before entering vendor
// initialization, outside loader lock and before the first real DXGI factory.
// 0=cold, 1=initializing, 2=initialized, 3=failed. No second initialization.
static std::atomic<unsigned> beta2VendorState{0};
extern "C" __declspec(dllexport) int __cdecl K033_Beta2InitializeVendor()
{
    unsigned expected=0;
    if(!beta2VendorState.compare_exchange_strong(expected,1))return expected==2?0:(expected==1?2:-1);
    if(!dllModule){beta2VendorState.store(3);return -1;}
    try {
        if(!Beta2VendorEntry(dllModule,DLL_PROCESS_ATTACH,nullptr)){beta2VendorState.store(3);return -1;}
        beta2VendorState.store(2);return 0;
    } catch(...) {beta2VendorState.store(3);return -1;}
}

BOOL APIENTRY DllMain(HMODULE hModule,DWORD reason,LPVOID reserved)
{
    if(reason==DLL_PROCESS_ATTACH){
        if(!yanyun033::HostSupported())return FALSE;
        DisableThreadLibraryCalls(hModule);
        dllModule=hModule;exeModule=GetModuleHandle(nullptr);processId=GetCurrentProcessId();
        return TRUE;
    }
    if(reason==DLL_PROCESS_DETACH&&beta2VendorState.load()==2)
        return Beta2VendorEntry(hModule,reason,reserved);
    return TRUE;
}
#endif
