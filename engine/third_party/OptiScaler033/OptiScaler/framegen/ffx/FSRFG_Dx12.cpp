...
ffxReturnCode_t FSRFG_Dx12::DispatchCallback(ffxDispatchDescFrameGeneration* params)
{
    ...
    // A function-static frame id leaked across instances and context rebuilds.
    // Frame zero is valid; a newly created context has no previous callback.
    if (_033Callback.Duplicate(params->frameID))
    {
        LOG_WARN("Dispatched with the same frame id! frameID: {}", params->frameID);
        params->numGeneratedFrames = 0;
        return FFX_API_RETURN_OK;
    }
    if (params->numGeneratedFrames == 0)
        return FFX_API_RETURN_OK;
    ...
    if (lastFGDisableHudless != Config::Instance()->FGDisableHudless.value_or_default())
    {
        ...
        if (_linkedHudlesDesc)
        {
            ...
            params->numGeneratedFrames = 0;
            _033Callback.Record(params->frameID);
    ...
    if (dispatchResult == FFX_API_RETURN_OK && params->numGeneratedFrames > 0 &&
        state.activeFgInput == FGInput::Upscaler)
        fgcompat033::Dispatched(GetTickCount64());
...
void FSRFG_Dx12::DestroyFGContext()
{
    ...
    // Teardown has stopped the old callbacks before the next context starts.
    _033Callback.Reset();
    ReleaseObjects();
}
