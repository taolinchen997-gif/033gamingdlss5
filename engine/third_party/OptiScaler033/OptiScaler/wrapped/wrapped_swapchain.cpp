...
static HRESULT LocalPresent(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags,
                            const DXGI_PRESENT_PARAMETERS* pPresentParameters, IUnknown* pDevice, HWND hWnd, bool isUWP, bool imageOnly033)
{
    ...
    if (willPresent)
    {
        ...
        // Draw overlay
        // Image-only FG uses the existing 033 panel rendered by ReShade. The
        // independent overlay's allocator ring assumes an ordinary swapchain;
        // it must not submit a second UI pass into this virtual backbuffer ring.
        if(!imageOnly033)MenuOverlayDx::Present(pSwapChain, SyncInterval, Flags, pPresentParameters, pDevice, hWnd, isUWP);
...
WrappedIDXGISwapChain4::WrappedIDXGISwapChain4(IDXGISwapChain* real, IUnknown* pDevice, HWND hWnd, UINT flags,
                                               bool isUWP, bool imageOnly033)
    : _real(real), _device(pDevice), _handle(hWnd), _refcount(1), _uwp(isUWP), _imageOnly033(imageOnly033)
{
    _id = ++scCount;
    _lastFlags = flags;
...
HRESULT STDMETHODCALLTYPE WrappedIDXGISwapChain4::Present(UINT SyncInterval, UINT Flags)
{
    ...
    {
        result = LocalPresent(_real, SyncInterval, Flags, nullptr, _device, _handle, _uwp, _imageOnly033);
                                                           ...
                                                           const DXGI_PRESENT_PARAMETERS* pPresentParameters)
{
    ...
    if ((Flags & DXGI_PRESENT_TEST) == 0)
    {
        result = LocalPresent(_real1, SyncInterval, Flags, pPresentParameters, _device, _handle, _uwp, _imageOnly033);
