...
{
    if(Core033::UsesHost()) {
        if(!cmdList || !params)return;
        const auto& cfg=*Config::Instance();
        if((cfg.RestoreComputeSignature.value_or_default() || cfg.RestoreGraphicSignature.value_or_default()) &&
            !D3D12Hooks::CanRestoreRootSignature(cmdList))return;
        ScopedNrStateEnvelope envelope(cmdList);
        Core033::OfferDx12(cmdList,params,timingQueue);return;
    }
    if (Core033::OfferDx12(cmdList,params,timingQueue)) return;
...
{
    // Capture retirement belongs to the same observer that protects the NR
    // submission. Never map a readback based on an arbitrary eight-frame delay.
    if(!Core033::RequestCapture())LOG_WARN("033 capture requires the active 033 DX12 NR renderer");
}
