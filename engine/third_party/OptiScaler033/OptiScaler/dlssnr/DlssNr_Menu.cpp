...
#include <integration/ControlsMenu033.h>
...
static void HelpMarker(const char* tip)
{
    ...
    if (Ui033::IsItemHovered())
    {
        Ui033::BeginTooltip();
        Ui033::PushTextWrapPos(Ui033::GetFontSize() * 40.0f);
        Ui033::TextUnformatted(tip);
        Ui033::PopTextWrapPos();
        Ui033::EndTooltip();
    }
}
                           ...
                           float def, const char* fmt = "%.2f")
{
    ...
    if (Ui033::SliderFloat(label, &value, mn, mx, fmt))
        pending[label] = value;

    if (Ui033::IsItemDeactivatedAfterEdit())
    {
    ...
    if (Ui033::SmallButton(resetId.c_str()))
    {
        *opt = def;
...
{
    if(Core033::UsesHost() && Core033::Owner()!=k033core::NativeVulkan) {
        if(auto ch=ScopedCollapsingHeader("033 Neural Rendering");ch.IsHeaderOpen()) {
            Core033::RenderControls(config);
        }
        return;
    }
    ...
    if (auto ch = ScopedCollapsingHeader("DLSS Neural Rendering"); ch.IsHeaderOpen())
    {
        ...
        if (Ui033::Checkbox("Enable Neural Rendering", &enabled))
        ...
        Ui033::TextDisabled("Can be toggled with a key -- bind it under Keybinds, \"Neural Rendering\".");

        bool applyModel = config->DlssNrApplyModel.value_or_default();
        if (Ui033::Checkbox("Apply the model", &applyModel))
        ...
        if (!enabled)
        {
            Ui033::TextDisabled("Off. The model stays loaded, so turning this back on is immediate.");
        }
        else if (!DlssNr::IsRunning() && !vulkan)
        {
            ...
            if (reason[0] != 0)
            {
                Ui033::TextColored(ImVec4(1.0f, 0.4f, 0.35f, 1.0f), "Off for this session: %s.", reason);
                Ui033::SameLine();

                if (Ui033::SmallButton("Retry"))
                    DlssNr::RetryAfterFailure();
            }
            else if (enabled)
                Ui033::TextUnformatted("Waiting for the upscaler to run.");
        }
        else
        {
            ...
            if (ms.has_value())
                Ui033::TextColored(ImVec4(0.4f, 0.9f, 0.5f, 1.0f), "Running%s - %.2f ms per frame%s",
                                   vulkan ? " natively on Vulkan" : "", ms.value(), runSuffix);
            else if (vulkan)
                // Measured but not yet read: the first few frames are still in the query ring.
                Ui033::TextColored(ImVec4(0.4f, 0.9f, 0.5f, 1.0f), "Running natively on Vulkan - %llu frames%s",
                                   DlssNr::FramesVk(), runSuffix);
            else
                Ui033::TextColored(ImVec4(0.4f, 0.9f, 0.5f, 1.0f), "Running.%s", runSuffix);

            Ui033::SameLine();
            Ui033::TextDisabled("(?)");
            if (Ui033::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled))
                Ui033::SetTooltip("The whole pass: the staging copies and the resolve as well as the"
        ...
        if (Ui033::SliderInt("Model resolution", &scalePercent, 25, 200, "%d%%"))
            pendingScale = scalePercent;

        if (Ui033::IsItemDeactivatedAfterEdit() && pendingScale >= 0)
        {
        ...
        if (scalePercent > 100)
            Ui033::TextDisabled("Supersampling %.2fx: the model runs ABOVE native, then\n"
        ...
        if (scalePercent > 100)
        {
            ...
            if (Ui033::Combo("Downscaler (NR)", &ds, dsNames, IM_ARRAYSIZE(dsNames)))
        ...
        // model's own picture and the two modes are identical, so the control says so by going grey.
        {
            ...
            if (Ui033::Combo("Enlargement", &enlarge, enlargeNames, IM_ARRAYSIZE(enlargeNames)))
        ...
        Ui033::SeparatorText("How much of it lands");

        float transfer = config->DlssNrTransferStrength.value_or_default();
        if (Ui033::SliderFloat("Detail strength", &transfer, 0.0f, 2.0f, "%.2f"))
            config->DlssNrTransferStrength = transfer;

        Ui033::SameLine();
        if (Ui033::SmallButton("Reset##detail"))
        ...
        if (Ui033::SliderFloat("Colour strength", &colour, 0.0f, 4.0f, "%.2f"))
            config->DlssNrColourStrength = colour;

        Ui033::SameLine();
        if (Ui033::SmallButton("Reset##colour"))
        ...
        if (reversible < 0 || reversible > 4)
            reversible = 0;
        if (Ui033::Combo("Reversible proxy (experimental)", &reversible, reversibleNames,
        ...
        Ui033::SeparatorText("Model");

        Ui033::TextUnformatted("Read when the model is built, so a change rebuilds it after a moment.");

        static const char* nrPresetNames[] = { "Default", "Preset 1", "Preset 2", "Preset 3" };
        int preset = (int) config->DlssNrPreset.value_or_default();
        if (Ui033::Combo("Model preset", &preset, nrPresetNames, IM_ARRAYSIZE(nrPresetNames)))
        ...
        if (Ui033::Combo("Style", &style, nrStyleNames, IM_ARRAYSIZE(nrStyleNames)))
        ...
        if (Ui033::Checkbox("Auto skin mask", &autoMask))
            config->DlssNrAutoMask = autoMask;

        HelpMarker("Lets the model find skin itself rather than treating the frame uniformly.");

        Ui033::SeparatorText("Colour");

        Ui033::TextDisabled("The model was trained on finished, sRGB-encoded frames. The upscaler's\n"
        ...
        // choice is made on what is available rather than on what sounds best.
        {
            ...
            if (Ui033::Combo("White point from", &source, sourceNames, IM_ARRAYSIZE(sourceNames)))
            {
            ...
            if (source == 1)
            {
                if (!vk && ex.seenFrames == 0)
                    Ui033::TextDisabled("Waiting for a frame...");
                else if (!haveExposure)
                    Ui033::TextColored(ImVec4(0.9f, 0.6f, 0.25f, 1.0f),
                                       "This game supplies no exposure -- paper white is in use. Try "
                                       "the scan instead.");
                else if (vk)
                    Ui033::TextColored(ImVec4(0.45f, 0.8f, 0.45f, 1.0f),
                ...
                else if (ex.exposure > 1e-6f)
                {
                    ...
                    Ui033::TextDisabled("Reading the exposure...");
            }
            else if (source == 2)
            {
        ...
        if (wpSource == 2)
        {
            ...
            if (showPaperWhite)
            {
                ...
                if (Ui033::SliderFloat(lbl, &pw, 0.25f, 2000.0f, "%.2fx", ImGuiSliderFlags_Logarithmic))
                {
                    if (editingRow)
                    {
            ...
            if (!anchors.empty())
            {
                ...
                if (Ui033::SliderFloat("Trim (x the scan)", &trim, 0.25f, 4.0f, "%.2fx",
                                       ImGuiSliderFlags_Logarithmic))
                    config->DlssNrScanTrim = std::clamp(trim, 0.25f, 4.0f);

                Ui033::SameLine();

                if (Ui033::SmallButton("Reset##scantrim"))
        ...
        else if (wpSource == 1)
        {
            ...
            if (Ui033::SliderFloat(ofScan ? "Trim (x the scan)" : "Trim (x the game's exposure)", &trim,
            ...
            if (Ui033::SmallButton("Reset##wptrim"))
            {
                if (ofScan)
                    config->DlssNrScanTrim = 1.0f;
                else
        ...
        else
        {
            ...
            if (Ui033::SliderFloat("Paper white", &wpScale, 0.25f, 2000.0f, "%.2fx",
                                   ImGuiSliderFlags_Logarithmic))
        ...
        if (Ui033::SliderFloat("Highlight guard", &maxRatio, 1.0f, 8.0f, "%.1fx"))
            config->DlssNrMaxRatio = maxRatio;

        Ui033::SameLine();
        if (Ui033::SmallButton("Reset##guard"))
        ...
        // reads, which left "Anchor here" looking like a control for something else entirely.
        {
            ...
            // eye adaptation.
                ...
                if (config->DlssNrWhitePointSource.value_or_default() == 2 &&
                    Ui033::Checkbox("Show the light meter on screen", &meter))
            ...
            if (DlssNr::ExposureScan::Scanning())
            {
                ...
                Ui033::BeginDisabled(live <= 0.0f || !isSource);

                if (Ui033::Button("Anchor here"))
                {
                ...
                if (!isSource)
                    Ui033::TextDisabled("(the scan is only watching -- the white point above comes "
                                        "from somewhere else)");
                ...
                if (!anchors.empty())
                {
                    ...
                    for (size_t i = 0; i < anchors.size(); ++i)
                    {
                        ...
                        if (Ui033::Selectable(row, sel))
                            selectedAnchor = sel ? -1 : (int) i;

                        Ui033::PopID();
                    }

                    Ui033::TextDisabled("Click a row to edit it with the slider above; click it again"
                ...
                if (anchors.size() == 1)
                {
                    ...
                    if (Ui033::Checkbox("The number runs the other way", &inverted))
                ...
                if (Ui033::TreeNode("Advanced"))
                {
                    ...
                    if (found.empty())
                    {
                        Ui033::TextDisabled("%s", why != nullptr && why[0] != 0
                                                      ? why
                                                      : "nothing matched yet.");
                    }
                    else
                    {
                        for (size_t i = 0; i < found.size(); ++i)
                        {
                            ...
                            if (c.reads == 0)
                            {
                                Ui033::TextDisabled("%zu. %s -- not read yet", i + 1, c.shape.c_str());
                                continue;
                            }

                            // Moving is the whole signal, so it is the thing that is coloured.
                            Ui033::TextColored(c.moves ? ImVec4(0.45f, 0.8f, 0.45f, 1.0f)
                        ...
                        Ui033::TextDisabled("Walk from shade into daylight. A real exposure moves.");
                        Ui033::TextDisabled("One that only ever climbs is a counter, not an exposure.");
                    }
        ...
        if (Ui033::Checkbox("Hold frame", &held))
        ...
        if (Ui033::Combo("Compare", &compare, compareNames, IM_ARRAYSIZE(compareNames)))
        ...
        if (compare != 0)
        {
            ...
            if (tags)
            {
                ...
                if (Ui033::SliderFloat("Label size", &tagScale, 0.5f, 5.0f, "%.1fx"))
        ...
        if (compare == 1)
        {
            ...
            if (Ui033::SliderFloat("Zoom", &zoom, 1.0f, 2.0f, "%.2f"))
        ...
        if (compare == 2)
        {
            ...
            if (Ui033::SliderFloat("Split", &split, 0.0f, 1.0f, "%.2f"))
        ...
        if (Ui033::Combo("Debug view", &debugView, debugNames, IM_ARRAYSIZE(debugNames)))
