...
NVSDK_NGX_API NVSDK_NGX_Result NVSDK_NGX_VULKAN_CreateFeature1_033Impl(VkDevice InDevice, VkCommandBuffer InCmdList,
...
NVSDK_NGX_API NVSDK_NGX_Result NVSDK_NGX_VULKAN_CreateFeature_033Impl(VkCommandBuffer InCmdBuffer,
                                                              NVSDK_NGX_Feature InFeatureID,
                                                              NVSDK_NGX_Parameter* InParameters,
                                                              NVSDK_NGX_Handle** OutHandle)
{
...
NVSDK_NGX_API NVSDK_NGX_Result NVSDK_NGX_VULKAN_ReleaseFeature_033Impl(NVSDK_NGX_Handle* InHandle)
{
    if (!InHandle)
        return NVSDK_NGX_Result_Success;
                                                                ...
                                                                PFN_NVSDK_NGX_ProgressCallback InCallback)
{
    ...
    if (handleId < DLSS_MOD_ID_OFFSET)
    {
        if (Config::Instance()->DLSSEnabled.value_or_default() && NVNGXProxy::VULKAN_EvaluateFeature() != nullptr)
        {
            ...
            if (result == NVSDK_NGX_Result_Success && k033core::Image(Core033::vkLedger.get(reinterpret_cast<uintptr_t>(InFeatureHandle)).feature))
    ...
    if (upscaleResult && !bridged && k033core::Image(Core033::vkLedger.get(reinterpret_cast<uintptr_t>(InFeatureHandle)).feature))
        DlssNr::EvaluateAfterUpscaleVk(InCmdList, InParameters, vkInstance, vkPD, vkDevice);
                                                               ...
                                                               NVSDK_NGX_Handle** OutHandle)
{
    ...
    const auto result=NVSDK_NGX_VULKAN_CreateFeature1_033Impl(InDevice,InCmdList,InFeatureID,InParameters,OutHandle);
    if(result==NVSDK_NGX_Result_Success && OutHandle && *OutHandle)
        Core033::vkLedger.created(reinterpret_cast<uintptr_t>(*OutHandle),true,metadata);
    return result;
}
...
{
    const auto metadata=Core033::ReadMetadata(static_cast<int>(InFeatureID),InParameters);
    const auto result=NVSDK_NGX_VULKAN_CreateFeature_033Impl(InCmdBuffer,InFeatureID,InParameters,OutHandle);
    if(result==NVSDK_NGX_Result_Success && OutHandle && *OutHandle)
        Core033::vkLedger.created(reinterpret_cast<uintptr_t>(*OutHandle),true,metadata);
    return result;
}
...
{
    const auto key=reinterpret_cast<uintptr_t>(InHandle);
    const auto result=NVSDK_NGX_VULKAN_ReleaseFeature_033Impl(InHandle);
    Core033::vkLedger.released(key,result==NVSDK_NGX_Result_Success);return result;
}
