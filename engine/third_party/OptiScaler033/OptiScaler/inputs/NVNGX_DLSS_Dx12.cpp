 ...
 */
NVSDK_NGX_API NVSDK_NGX_Result NVSDK_NGX_D3D12_CreateFeature_033Impl(ID3D12GraphicsCommandList* InCmdList,
                                                             ...
                                                             NVSDK_NGX_Handle** OutHandle)
{
    ...
    if(InFeatureID==NVSDK_NGX_Feature_FrameGeneration){
        const bool entry=Nvngx_FG::isDx12Available()||NVNGXProxy::D3D12_CreateFeature()!=nullptr;
        fgroute033::nativeCapability.Observe(entry?fgcap033::Ready:fgcap033::Incompatible);
        if(fgroute033::UniversalBoot()){
            *OutHandle=nullptr;return NVSDK_NGX_Result_FAIL_FeatureNotSupported;
        }
    }
    ...
    // 2026-09-12 评论区（赛博朋克2077）：「光线重构和路径追踪冲突了，只要打开这个 dlss5 就用不了」。
    // 根因就在这一行：光线重建(13)以前跟超分一起被排除在原生透传之外，改用 OptiScaler 自己的
    // DLSSD 顶替游戏的 RR。033 的活是【骑在游戏自己的 DLSS 上做神经渲染】，不是替它实现光线重建；
    // 顶替既没必要，还会把 2077 这种把 RR 和路径追踪绑在一条管线上的游戏整条弄不能用。
    // 现在跟其它非超分特性一样原样透传给 NVIDIA 运行库。★NR 一点不丢★：求值的原生透传分支本来
    // 就会对 k033core::Image(feature)（1 和 13 都算）调 DlssNr::EvaluateAfterUpscale，而外层
    // CreateFeature 包装按【请求的 feature id】记账本，走哪条路都记，求值时 Scope 再取回来。
    if (InFeatureID != NVSDK_NGX_Feature_SuperSampling)
    {
        ...
        // 原生运行库压根不可用时（DLSSEnabled 关 / InitDx12 失败 / 拿不到函数指针），光线重建退回
        // OptiScaler 自己的 DLSSD —— 不能因为透传走不通就把游戏的光线重建直接弄没。
        if (InFeatureID != NVSDK_NGX_Feature_RayReconstruction)
            return NVSDK_NGX_Result_FAIL_FeatureNotSupported;
        LOG_WARN("Ray reconstruction: native runtime unavailable, falling back to OptiScaler's own DLSSD");
    }

    // OptiScaler internal handling (SuperSampling, or ray reconstruction with no native runtime)
...
NVSDK_NGX_API NVSDK_NGX_Result NVSDK_NGX_D3D12_ReleaseFeature_033Impl(NVSDK_NGX_Handle* InHandle)
{
    LOG_FUNC();
    ...
    NVSDK_NGX_FeatureRequirement* OutSupported)
{
    ...
    if(isFG){
        const bool entry=dlssgAdjacent||NVNGXProxy::D3D12_GetFeatureRequirements()!=nullptr;
        fgroute033::nativeCapability.Observe(entry?fgcap033::Ready:fgcap033::Incompatible);
        // Querying FG support may load the native provider. Preserve SR queries
        // but reject this ownership request before calling that runtime.
        if(fgroute033::UniversalBoot())return NVSDK_NGX_Result_FAIL_FeatureNotSupported;
    }
                                               ...
                                               PFN_NVSDK_NGX_ProgressCallback InCallback)
{
    ...
    bool evalSuccess = false;
    {
        ...
        feature->EvaluationResult=NVSDK_NGX_Result_Fail;
        evalSuccess = feature->Evaluate(InCmdList, InParameters);
        k033core::context.completed=evalSuccess;
        if(evalSuccess){
            k033core::context.info.outputW=feature->TargetWidth();
            k033core::context.info.outputH=feature->TargetHeight();
        }
    }
    ...
    if (!evalSuccess)
    {
        ...
        static srnotice033::Gate notice;
        if(notice.Notify(GetTickCount64()))
            ImGui::InsertNotification({ ImGuiToastType::Error, 10000, "超分处理失败，详情已记录。持续出现时请反馈。" });
    }
    ...
    return evalSuccess ? NVSDK_NGX_Result_Success :
        (feature->EvaluationResult==NVSDK_NGX_Result_Success?NVSDK_NGX_Result_Fail:feature->EvaluationResult);
}
 ...
 */
NVSDK_NGX_API NVSDK_NGX_Result NVSDK_NGX_D3D12_EvaluateFeature_033Impl(ID3D12GraphicsCommandList* InCmdList,
                                                               const NVSDK_NGX_Handle* InFeatureHandle,
                                                               NVSDK_NGX_Parameter* InParameters,
                                                               PFN_NVSDK_NGX_ProgressCallback InCallback)
{
    ...
    const auto knownFeature = HandleToFeature.find(handleId);
    const auto feature = knownFeature == HandleToFeature.end() ? static_cast<NVSDK_NGX_Feature>(-1) : knownFeature->second;
    ...
    if (handleId < DLSS_MOD_ID_OFFSET)
    {
        if (cfg.DLSSEnabled.value_or_default() && NVNGXProxy::D3D12_EvaluateFeature() != nullptr)
        {
            ...
            k033core::context.completed=result==NVSDK_NGX_Result_Success;
            ...
            if (result == NVSDK_NGX_Result_Success && k033core::Image(k033core::context.info.feature))
    ...
    if (optiResult == NVSDK_NGX_Result_Success && k033core::Image(k033core::context.info.feature))
        DlssNr::EvaluateAfterUpscale(InCmdList, InParameters);
...
NVSDK_NGX_API NVSDK_NGX_Result NVSDK_NGX_D3D12_CreateFeature(ID3D12GraphicsCommandList* InCmdList,
                                                             ...
                                                             NVSDK_NGX_Handle** OutHandle)
{
    const auto metadata=Core033::ReadMetadata(static_cast<int>(InFeatureID),InParameters);
    const auto result=NVSDK_NGX_D3D12_CreateFeature_033Impl(InCmdList,InFeatureID,InParameters,OutHandle);
    if(result==NVSDK_NGX_Result_Success && OutHandle && *OutHandle)
        Core033::dx12Ledger.created(reinterpret_cast<uintptr_t>(*OutHandle),true,metadata);
    return result;
}
...
NVSDK_NGX_API NVSDK_NGX_Result NVSDK_NGX_D3D12_EvaluateFeature(ID3D12GraphicsCommandList* InCmdList,
                                                               ...
                                                               PFN_NVSDK_NGX_ProgressCallback InCallback)
{
    k033core::Scope scope(Core033::dx12Ledger.get(reinterpret_cast<uintptr_t>(InFeatureHandle)),reinterpret_cast<uintptr_t>(InFeatureHandle),k033core::Dx12);
    return NVSDK_NGX_D3D12_EvaluateFeature_033Impl(InCmdList,InFeatureHandle,InParameters,InCallback);
}

NVSDK_NGX_API NVSDK_NGX_Result NVSDK_NGX_D3D12_ReleaseFeature(NVSDK_NGX_Handle* InHandle)
{
    const uintptr_t key=reinterpret_cast<uintptr_t>(InHandle);
    const auto id=InHandle?InHandle->Id:0;
    const auto result=NVSDK_NGX_D3D12_ReleaseFeature_033Impl(InHandle);
    Core033::dx12Ledger.released(key,result==NVSDK_NGX_Result_Success);
    if(InHandle && result==NVSDK_NGX_Result_Success)HandleToFeature.erase(id);
    return result;
}
