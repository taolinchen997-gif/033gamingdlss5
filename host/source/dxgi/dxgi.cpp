...
static void init_swapchain_proxy(IDXGIFactory *factory, reshade::api::device_api direct3d_version, const com_ptr<IUnknown> &device_proxy, T *&swapchain, DXGI_USAGE usage, [[maybe_unused]] UINT sync_interval, [[maybe_unused]] DXGI_SWAP_CHAIN_DESC orig_desc, [[maybe_unused]] bool desc_modified)
{
    ...
    // The 033 image-only virtual swapchain supplies its sole explicit runtime.
    // The versioned marker is forwarded by the controlled core wrapper; no
    // heuristic DLL/name/size test is used to classify another game's chain.
    static constexpr GUID k033UniversalOwnerGuid = {0x39480332,0xc11e,0x4e8c,{0x93,0xf3,0x01,0x6b,0x8e,0x37,0x12,0x23}};
    UINT ownerVersion=0, ownerSize=sizeof(ownerVersion);
    if(SUCCEEDED(swapchain->GetPrivateData(k033UniversalOwnerGuid,&ownerSize,&ownerVersion)) &&
       ownerSize==sizeof(ownerVersion) && ownerVersion==0x03320001u)
        return;
#endif
...
{
	if (k033_ensure_core_loaded())
		k033_register_renderer();
