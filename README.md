...
《燕云十六声》专用的画质插件（作者 B站 @热心网友033）：借用游戏自带的 DLSS 做神经降噪（NR），外加分层超分（SR）、前置调色、人物 / 场景分开调节和帧生成（FG）。

本仓库是 **V6.2** 安装包里所有由 033 编译的程序的完整源码，用来履行 GPL-3.0 / AGPL-3.0 的源码义务。

> **安装包不在 GitHub 上。** 包里带有 NVIDIA 的运行库，不适合放在这里。安装包请到作者的 B站主页下载：[B站 @热心网友033](https://space.bilibili.com/88101991)。
...
按 [BUILD.md](BUILD.md) 编出来的转发器、人物识别、入口和 V6.2 安装包里的文件逐字节一致，只差编译时间戳（.NET 启动器还有一个每次编译都会变的模块 ID）；核心换个目录编时，MSVC 会按源码路径改动一部分数据的排列，核对方法和结果见 BUILD.md。

| 文件 | V6.2 安装包里的 SHA-256 |
|---|---|
| `033-runtime/033-engine.dll` | `3bf7d7f04c3c71929073e54f164759c85a7e1874be3b8191ac8d19d1a3bc28c0` |
